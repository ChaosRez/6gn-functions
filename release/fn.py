...
def fn(input: typing.Optional[str]) -> typing.Optional[str]:
    ...
    with tracer.start_span('select_mutated_elems', attributes={"invoke_count": Counter.get_count()}):
    ...
    with tracer.start_span('publish_w_delivery_report', attributes={"invoke_count": Counter.get_count()}):
    ...
    with tracer.start_span('post_update', attributes={"invoke_count": Counter.get_count()}):
        post_update(mutated_data, meta)

    # Wait for any outstanding messages to be delivered and delivery reports to be received.
    with tracer.start_span('flush', attributes={"invoke_count": Counter.get_count()}):
        p.flush()  # blocking
